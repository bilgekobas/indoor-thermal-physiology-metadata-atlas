  ...
  return (
    <div className="overflow-x-auto">
      ...
      <svg width={W + 20} height={H + 20} className="font-data overflow-visible">
        <line x1={xScale(0)} x2={xScale(0)} y1={0} y2={H} stroke="#B8C020" strokeWidth={1} opacity={0.5} />
        {filtered.map((r, i) => {
          const y = i * rowH + rowH / 2
          const flagged = Math.abs(r.diff) > flagThreshold
          return (
            <circle key={r.id} cx={xScale(r.diff)} cy={y} r={flagged ? 2.4 : 1.4}
              fill={flagged ? '#D94F6E' : '#A8A59C'}
        ...
        {Array.from({ length: domainMax - domainMin + 1 }, (_, i) => domainMin + i).filter((v) => v % 2 === 0).map((v) => (
          <text key={v} x={xScale(v)} y={H + 14} fontSize={10} fill="#A8A59C" textAnchor="middle">{v > 0 ? `+${v}` : v}</text>
        ))}
      </svg>
        ...
        {studies.map((s, i) => {
          ...
          return (
            <g key={i}
              ...
              <rect x={x} y={malePx} width={barW} height={H - malePx} fill="#D94F6E" />
            </g>
          )
        })}
      </svg>
      <div className="flex gap-4 mt-2 text-[11px] text-inkmid">
        <span className="flex items-center gap-1.5"><span className="w-2.5 h-2.5 inline-block rounded-sm" style={{ background: '#E2DED4' }} /> Male</span>
        <span className="flex items-center gap-1.5"><span className="w-2.5 h-2.5 inline-block rounded-sm" style={{ background: '#D94F6E' }} /> Female</span>
      </div>
      <TooltipPortal tip={tip} />
    </div>
  )
  ...
  return (
    <div>
      <ChapterHeader
        ...
        headline={[
          { value: 22, label: 'Median sample size', color: '#E07820' },
          { value: `${sexSummary.equal}`, label: 'studies with balanced sex split', color: '#4855C8' },
          { value: '~70%', label: '"healthy" used with no stated test', color: '#D94F6E' },
        ]}
      />
      ...
      >
        <FigureCard figNumber="8" title="Participant age" commentary={`Mean ± SD per study, sorted ascending. ${fig08_age.studies.filter((s) => !s.std_reported).length} studies report a mean with no SD (shown as a point).`}>
          <StudyIntervalPlot
            ...
            domain={[15, 60]} color="#E07820" unit="" xAxisLabel="Age (years)"
          />
        </FigureCard>
        ...
        <FigureCard figNumber="9" title="Participant BMI" commentary={`Mean ± SD per study, sorted ascending. ${fig09_bmi.studies.filter((s) => !s.std_reported).length} studies report a mean with no SD.`}>
          <StudyIntervalPlot
            ...
            domain={[16, 30]} color="#B8C020" unit="" xAxisLabel="BMI (kg/m²)"
          />
        </FigureCard>
