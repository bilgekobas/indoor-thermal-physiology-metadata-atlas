...
export default function ChapterContext({ data }) {
  ...
  // above (which is a share of studies). Uses total_corpus_participants
  // (computed server-side over every study with a reported sample size,
  // before the >=3-studies-per-country display filter) as the denominator --
  // NOT a sum over sample_size_by_country.studies, which only contains the
  // subset of countries large enough to display individually and would
  // silently understate the true total (e.g. this previously showed China
  // at 74.4% of a partial sum instead of 69.9% of the true corpus total).
  const totalParticipants = sample_size_by_country.total_sample_size_all_countries
